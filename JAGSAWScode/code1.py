# -*- coding: utf-8 -*-
"""
Created on Thu Jun 28 12:56:14 2018
...
''' Search all matched string '''
def find_pattern(word,pattern):
	tt = re.search(r''+pattern,word)
	if tt is None:
		print("What the hell here!")
		return None
	return tt.group(0)
...
def split_input_for_training(sequence):
	# get number of hands 
	num_hands= len(input_shapes) # = 4
	# get number of dimensions cluster for each hand 
	num_dim_clusters = len(input_shapes[0]) # = 5
...
''' shuffles train and labels, Xao tron du lieu '''
...
def getMetaDataForSurgeries(surgery_type):
	...
	for line in file: 
		...
		b = line.split()
		surgery_name = b[0]   # Take name of the surgery task: Suturing, Knot-typing or Needle Passing
		expertise_level = b[1] # Take level: N, I or E
		b = b[2:]
...
def generateMaps(surgery_type):
	...
	for subdir,dirs,files in os.walk(path):
		for file_name in files: 
			surgery = readFile(path + file_name,float,columns_to_use = dimensions_to_use) # dimensions_to_use = 76
			surgery_name = file_name[:-4]
			expertise_level = getExpertiseLevelOfSurgery(surgery_name) # Compare with meta_file to know the level of surgery
	...
	for it in range(0,max_itr):
		for subdir,dirs,files in os.walk(path):
			...
			if(len(files)>0):
				...
				fold2 = find_pattern(subdir,'SuperTrialOut'+'/.*_Out')
				print("subdir = ", subdir)
				print("fold2 = ", fold2)
				fold1 = fold2.replace('SuperTrialOut'+'/','')
				fold = fold1.replace('_Out','')
...
''' Evaluate the model by calculate accuracy, recall and precision'''
...
''' Takes 60 images to create gif files'''
